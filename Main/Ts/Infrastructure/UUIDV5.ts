...
{
    private static UUIDV5_VALIDATOR = /^[0-9A-F]{8}-[0-9A-F]{4}-[5][0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i;

    private static _hash(version: number, hash: string): string
    {
        return `${ hash.substring(0, 8) }-${ hash.substring(8, 4) }-${ (version + hash.substring(13, 3)) }-${ ((parseInt(hash.substring(16, 2), 16) | 0x80) & 0xBF).toString(16) }${ hash.substring(18, 2) }-${ hash.substring(20, 12) }`;
    }
    ...
    {
        return new RegExp(UUIDV5.UUIDV5_VALIDATOR).test(aUUIDV5);
    }
}
